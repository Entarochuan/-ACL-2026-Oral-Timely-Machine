...
class SGLangRollout(BaseRollout):
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for result in tool_call_results:
                        if len(result) != 3:
                            # 处理现有的二元组错误格式
                            print("Warning: tool call result should be a 3-tuple, got", result)
                            processed_results.append((str(result), 0.0, {}))
                        else:
                            # 正常情况
                            # print("Correctly processing tool call result:", result)
                            processed_results.append(result)

                    tool_call_results = processed_results
    ...
    async def _handle_pending_state(self, _req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        if _req.tool_schemas is not None:
            ...
            for result in tool_creation_results:
                if len(result) == 2:
                    # 正常情况：2元组 (_, tool_result)
                    _, tool_result = result
                    processed_tool_results.append(tool_result)
                elif len(result) == 3:
                    # 兼容3元组格式 (tool_result, _, _)
                    _, _, tool_result = result
                    processed_tool_results.append(tool_result)
                else:
                    # 其他格式：直接使用第一个元素或整个结果
                    processed_tool_results.append(result[0] if len(result) > 0 else result)
            
            _req.add_tool_response_messages(
                self.processing_class, processed_tool_results
            )
