...
class AsyncRolloutRequest(BaseModel):
    ...
    def add_tool_response_messages(
        self,
        processing_class: PreTrainedTokenizer | PreTrainedTokenizerFast | ProcessorMixin,
        contents: list[ToolResponse | str],
    ) -> None:
        # Normalize contents: convert str to ToolResponse for backward compatibility
        normalized_contents = []
        for content in contents:
            if isinstance(content, str):
                normalized_contents.append(ToolResponse(text=content))
            elif isinstance(content, ToolResponse):
                normalized_contents.append(content)
            else:
                raise TypeError(f"Expected ToolResponse or str, got {type(content)}")
        
        if not normalized_contents or all(content.is_empty() for content in normalized_contents):
            return
        ...
        messages = [*BASE_CHAT_HISTORY, *self.messages[-len(normalized_contents) :]]
        tools = [tool.model_dump() for tool in self.tool_schemas] if self.tool_schemas else None
